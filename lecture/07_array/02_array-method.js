...
console.log(seasons.join('/'));
console.log('------------');
const irt = seasons.entries();
console.log(itr);

for(let obj of itr){}

//결론 
// indexOf : Array 탐색 > .indexOf('찾을요소',시작위치) - 시작위치 생략가능
//lastIndextoOf : 뒤에서부터 , 
//include : 포함여부
//pop():마지막 요소 제거. 빈 배열은 undefined 반환
//push():
//shift : 맨앞 요소 추가 // unshift('요소') : 맨 앞에 요소 추가
//배열1.concact(합칠배열2) : 합친 배열 만들어짐
//reverse(): 역순으로 저장
//[...mix1].reverse() : 역순의 카피본 만들기
//slice(시작범위,끝범위) : 범위 사이 요소로 배열 또 만들기
//배열.toString() : 안에있는 글자만 빼내기
//배열.join():()안에 있는걸로 구분하는 하나의 분자열로 반환