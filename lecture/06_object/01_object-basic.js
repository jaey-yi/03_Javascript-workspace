...
let person2 = {
  ...
  getInfo: function(){
    return `p${this.name}은  ${this.age}세 입니다`
  }
};
...
// 객체 리터럴 = 데이터를 키 -쌍 구조화 해서 관리하는 기본적-직관적 방법
...
const movie = {
  ...
  toString:function(){
    // 메소드 내에서 현 객체의 프로퍼티 참조할 경우 this. 활용
    // 화살표 함수 내에서는 this 가 객체로 바인딩 되지 않음
...
/* 
* in 연산자 
1) 특정객체 내에 특정 프로퍼티가 존재하는 지 여부를 확인할 수 있는 연산자
2) 사용법
...
for(let key in movie){
  console.log(movie[key]); // 여기서 말하는거. 프로퍼티 접근시 = for, 부를때 [대괄호] 사용
}
...
결론 : 객체/오브젝트. 프로퍼티-메소드.
객체 = 서랍장
프로퍼티 = 서랍장 안의 각 서랍 (이름표 + 내용물)
매소드 = 객체 내에 서랍으로 함수를 할당 한 것
 객체/오브젝트는 전체 덩어리, 프로퍼티는 그 안의 개별 요소들이다. 
in 연산자 : console.log('test' in movie); >  movie 객체(서랍)에 test 프로퍼티(내용물) 있으면 true
객체 : 객체  - 프로퍼티(키+값)
             - 메소드  (키+함수) 
*/
